...
When you are done with your graph nodes and relationships, you label them and define them by using the so called properties. You know that pancakes are food but they can also be used for making a cake. So, you add two labels next to "pancake" - "food" and "ingredient". Then, you connect the pancake to its ingredients by adding the relationship "contains". You can further model the link by assigning the property "quantity" to it, which can be also related to the recipe. Then, you define the node _recipe_ by adding the following properties to it. You can take them from the real recipe.
