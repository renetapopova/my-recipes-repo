...
When you finish defining your graph nodes and relationships, you label them and specify them by using the so-called properties. You know that pancakes are food but they can also be used for making a cake. So, you add two labels next to _pancake_: _Food_ and _Ingredient_. Then, you connect the pancake to its ingredients by adding the relationship _contains_. You can further model the link by assigning the property `quantity:' '` to it, which can also be related to the node _recipe_. Then, you specify the nodes that have labels _Food_ and _Ingredient_ by adding the property `name:' '` to them.
